...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (min_price !== undefined && isNaN(min_price)) {
      return new Response(
        JSON.stringify({ success: false, error: "min_price must be a number" }),
        { status: 400, headers }
      );
    }
    ...
    if (max_price !== undefined && isNaN(max_price)) {
      return new Response(
        JSON.stringify({ success: false, error: "max_price must be a number" }),
        { status: 400, headers }
      );
    }
    ...
    if (isNaN(limit) || limit <= 0 || limit > 50) {
      return new Response(
        JSON.stringify({ success: false, error: "limit must be a number between 1 and 50" }),
        { status: 400, headers }
      );
    }
    ...
    if (isNaN(page) || page < 1) {
      return new Response(
        JSON.stringify({ success: false, error: "page must be a positive number" }),
        { status: 400, headers }
      );
    }
